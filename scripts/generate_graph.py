...
def main():
    ...
    parser.add_argument("--csv_path", type=str, default='../data/nisq/cswap/n_trgts=1,2,3,4,5,6-p2=0.001,0.003,0.005-method=telegate.csv')
    parser.add_argument("--save_path", type=str, default='../data/nisq/cswap_graphs/telegate_simulations.pdf')
    ...
    for idx, p2_val in enumerate(sorted(df['p2'].unique())):
        ...
        p_opt, _ = curve_fit(fidelity_model, n_vals, fid_vals, p0=[1.0, 0.05])
        ...
        if idx == 0:
            plt.plot(n_fit, fid_fit, linestyle='--', label='Fit', c=colors[idx], alpha=0.6)
        else:
    ...
    plt.title('Mean Fidelity of Two-Party CSWAP (Telegate Scheme)', fontsize=13)
    plt.ylim(0.85, 1.01)
